...
def init(num_vehicles, vo):
    ...
    vehicles = []
    controllers = []
    controller_states = [ControllerState(0, 0)]
    vehicle_types = generate_vehicle_types(num_vehicles)

    for i in range(num_vehicles):
        controller_states = [ControllerState(0, 0)]  # Lista di stati per ogni controllore
        controllers.append(Controller(controller_states))

        do = (vo * 3.6 / 10) ** 2  # distanza di sicurezza che vogliamo mantenere
        if i == 0:
    ...
    for k in range(0, 10):  # esempio numero di campionamenti
    ...
    for j in range(num_vehicles):  # CICLO DI STAMPE
        ...
        for k in range(10):
            ...
            print("distance: ", vehicles[j].states[k].distance, ", velocity: ", vehicles[j].states[k].velocity, ", acceleration: ", vehicles[j].states[k].acceleration, ", position: ", vehicles[j].states[k].position)

    print("--------------------\n")


def generate_vehicle_types(num_vehicles):   # generatore di tipi di veicoli con il triplo di probabilità di macchine rispetto a bus
    car_probability = 3
    bus_probability = 1
